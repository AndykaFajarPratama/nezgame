...
import { User, Mail, Lock, ArrowRight, AlertCircle, Eye, EyeOff, RefreshCw } from "lucide-react";
...
export default function Register() {
  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    try {
      // Use custom endpoint to prevent immediate database entry
      const response = await fetch(`${import.meta.env.VITE_API_BASE_URL || ""}/api/auth/custom/register-request`, {
        method: "POST",
      ...
      if (!response.ok) {
        ...
        // Success - Request OTP and redirect
        ...
        <div className="glass p-10 rounded-3xl border border-white/5 relative z-10 shadow-2xl">
          <div className="flex flex-col items-center mb-10">
            ...
            <h1 className="text-3xl font-heading font-black text-white tracking-tighter">
              DAFTAR <span className="text-cyan-400">AKUN</span>
            </h1>
            <p className="text-slate-500 text-[10px] font-bold uppercase tracking-[0.3em] mt-2">Lengkapi data untuk pendaftaran</p>
          </div>
          ...
          <form onSubmit={handleRegister} className="space-y-5">
            ...
            <div className="space-y-2">
              <label className="text-[10px] font-bold text-slate-500 uppercase tracking-widest pl-1">Kata Sandi</label>
              <div className="relative group">
                <div className="absolute inset-y-0 left-4 flex items-center pointer-events-none text-slate-600 group-focus-within:text-blue-500 transition-colors">
            ...
            <div className="space-y-2">
              <label className="text-[10px] font-bold text-slate-500 uppercase tracking-widest pl-1">Konfirmasi Sandi</label>
              <div className="relative group">
                <div className="absolute inset-y-0 left-4 flex items-center pointer-events-none text-slate-600 group-focus-within:text-blue-500 transition-colors">
                  <Lock className="w-4 h-4" />
                </div>
            ...
            >
              ...
              <div className="relative py-4 flex items-center justify-center gap-2 text-white font-black text-xs uppercase tracking-widest">
                {loading ? <RefreshCw className="w-4 h-4 animate-spin" /> : <>Daftar Sekarang <ArrowRight className="w-4 h-4 group-hover:translate-x-1 transition-transform" /></>}
              </div>
            </button>
          </form>
          ...
          <div className="mt-8 pt-8 border-t border-white/5 text-center">
             <p className="text-slate-500 text-sm">
               Sudah punya akun?{" "}
               <Link to="/login" className="text-blue-500 font-bold hover:text-cyan-400 transition-colors">Masuk Sekarang</Link>
             </p>
          </div>
        </div>
      </div>
